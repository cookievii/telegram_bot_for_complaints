...
from core.messages import (APPLICATION, ASK_ADDRESS, ASK_COMPLAINT,
                           ASK_DESCRIPTION, ASK_MEDIA, BACK, ERROR_NON_TEXT,
                           ERROR_PHOTO, GREETING, LEAVE_REQUEST, OFFER_MESSAGE,
                           SEND_COMPLAINT, SEND_OFFER, SHARE_IN_OFFER, SKIP,
...
def register_handlers_complaint(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(get_callback_address, state=ConfirmForm.address)
    ...
    dp.register_message_handler(
        get_non_media_offer, content_types=ContentType.ANY, state=OfferForm.description
    )
